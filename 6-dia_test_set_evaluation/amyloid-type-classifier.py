...
# Normalize legacy codename/type abbreviation for transthyretin.
TYPE_RENAMES = {"THY": "TTR"}

def normalize_codename(cn):
    if not isinstance(cn, str):
        return cn
    return cn.replace("DDA-THY", "DDA-TTR").replace("DIA-THY", "DIA-TTR")
...
def extract_type_from_codename(cn):
    cn = normalize_codename(cn)
    if not isinstance(cn, str) or len(cn) < 7:
        return ""
    raw_type = cn[4:7].upper()
    return TYPE_RENAMES.get(raw_type, raw_type)
...
for fname in files:
    ...
    for _, r in df.iterrows():
        rows.append({
            "CodeName": normalize_codename(r[rep]),
            "Feature": r["Feature"],
            "MS2": r["MS2"]
        })
...
preds = [TYPE_RENAMES.get(x, x) for x in encoder.inverse_transform(pred_idx)]
...
for i, cls in enumerate(encoder.classes_):
    df_pred[f"P_{TYPE_RENAMES.get(cls, cls)}"] = probs[:, i]
